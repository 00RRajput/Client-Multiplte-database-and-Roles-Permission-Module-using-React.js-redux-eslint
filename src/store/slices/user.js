...
const slice = createSlice({
    ...
    reducers: {
        ...
        getFormFields(state, action) {
            state.formFiels = action.payload;
        }
    }
});
...
export function getFields(client = '') {
    return async () => {
        try {
            const response = await axios.get(`/users/user-fields/${client}`);
            console.log('response.data', response.data);
            // dispatch(slice.actions.getFormFields(response.data.users_s1));
        } catch (error) {
