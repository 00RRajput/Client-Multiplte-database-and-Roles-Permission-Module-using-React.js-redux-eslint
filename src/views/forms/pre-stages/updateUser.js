...
function UpdateUser({ edit, data }) {
    ...
    useEffect(() => {
        ...
        dispatch(getUserConfigFields(user?.client_id));
        ...
        if (fields?.visibility?.department) {
            roles.filter((role) => {
                if (role.department.length) {
                    if (role.department[role.department.length - 1].department === data?.department) return role;
                } else return role;
                return undefined;
            });
            setMultiRoles(roles);
        } else setMultiRoles(roles);

        const roleId = data.role.map((item) => item.id);
        formik.setValues({
            name: data.name,
            ...
            relation_contact: data?.relation_contact,
            password: '',
            confirm_password: ''
        });
        // const results = [];
            ...
            <MainCard title="Update User">
                <form onSubmit={formik.handleSubmit}>
                    <Grid container spacing={gridSpacing}>
                        ...
                        {/* <Grid item xs={12} md={3}>
                            <Stack>
                                <InputLabel required>Select Hub</InputLabel>
                                <Select
                                    id="hub"
                                    name="hub"
                                    label="Select hub"
                                    defaultValue="Select hub"
                                    value={formik.values.hub || 'Select hub'}
                                    onChange={formik.handleChange}
                                >
                                    <MenuItem value="Select hub" selected disabled>
                                        Select hub
                                    </MenuItem>
                                    <MenuItem value="hello">hello</MenuItem>
                                </Select>
                                {formik.errors.hub && <FormHelperText error>{formik.errors.hub}</FormHelperText>}
                            </Stack>
                        </Grid> */}
