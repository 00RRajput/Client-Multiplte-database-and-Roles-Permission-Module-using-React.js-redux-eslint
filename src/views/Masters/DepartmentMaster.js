...
const EditDepartment = Loadable(lazy(() => import('views/forms/department/EditDepartment')));
// table sort
...
const DepartmentMaster = () => {
    ...
    const handleActive = async (id, isActive, deprtmentName) => {
        await axios.put(`/departments/${id}`, { client: user?.client, department: deprtmentName, isActive: !isActive });
        dispatch(
            openSnackbar({
                open: true,
                message: `Department ${isActive ? 'disabled' : 'enabled'} successfully !`,
                variant: 'alert',
                alert: {
                    color: 'success'
                },
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values, { resetForm }) => {
            if (values) {
                try {
                    setOpen(true);
                    await axios.post('/departments', formik.values);
                    resetForm();
                    handleLoad();
                    dispatch(
                        openSnackbar({
