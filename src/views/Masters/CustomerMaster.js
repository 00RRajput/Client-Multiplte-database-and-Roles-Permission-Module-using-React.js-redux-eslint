...
const CustomerMaster = () => {
    ...
    const handleSearch = (event) => {
        ...
        if (newString) {
            const newRows = rows.filter((row) => {
                let matches = true;
                const properties = ['vehicle_type', 'l_feet', 'b_feet', 'h_feet', 'capacity_kgs', 'cft', 'cbm'];
    ...
    const handleActive = async (id, isActive) => {
        ...
        await axios.put(`/customer/${id}`, { isActive: !isActive }).then((response) => {
            dispatch(
                openSnackbar({
                    open: true,
    ...
    return (
        <MainCard
            ...
            secondary={
                <Grid item>
                    <PermissionGuard access="create_customer">
                                    ...
                                    >
                                        ...
                                        <TableCell align="left">
                                            <Grid container spacing={1}>
                                                {row.locations.map((item, index) => (
                                                    <Grid item key={index}>
                                                        <Chip label={item.location_name} color="success" />
                                                    </Grid>
                                                ))}
                                                {/* {row.location_name.length > 2 && (
                                                    <Grid item>
                                                        <Button variant="outlined" color="primary" size="small" onClick={handleSeeMore}>
                                                            See More
                                                        </Button>
                                                    </Grid>
                                                )}
                                                {view &&
                                                    row.location_name.map((item, index) => (
                                                        <Grid item key={index}>
                                        ...
                                        <TableCell
                                            ...
                                            sx={{
                                                color:
                                                    row?.prestages === 'available' ? theme.palette.success.main : theme.palette.error.main
                                            }}
                                        >
                                            {capitalizeFirstLetter(row?.prestages)}
                                        </TableCell>
                                        <TableCell align="center" sx={{ pr: 3 }}>
                                            <PermissionGuard access="delete_customer">
                                                <Switch
                                            ...
                                            <PermissionGuard access="edit_customer">
                                                <IconButton
                                                    color="secondary"
                                                    size="small"
                                                    aria-label="edit"
